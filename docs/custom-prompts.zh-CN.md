...
自定义命令只要 `name` 和内置的一致，就会**替换掉它**，并保留它在斜杠菜单里的位置。所以想微调
`/Summarize`，办法是从[内置 prompt 全文](./built-in-prompts.zh-CN.md)里把它的 prompt 复制出来，
改成你要的样子，再作为一条名为 `Summarize` 的自定义命令加进去。

但要清楚这意味着什么。这些 prompt 经过了好几轮有实测依据的修正，而且措辞远比看上去脆弱 ——
一句关于"用什么语言回答"的话，曾让五条命令把英文块答成中文；一个多写了一个从句的占位符，
被模型原样抄进了用户的笔记；把 `/Fact Check` 里两句近似重复的规则合并，又把已经修好的毛病带了回来。
`docs/development.zh-CN.md` 里介绍了用来发现这类问题的 live 套件。

**被你覆盖的命令不再收到修复。** 你的副本停在复制它的那一天；之后对那条命令的所有修正会到达除你以外的
每个人，而且没有提示。删掉你的版本就能回到维护中的那条。
